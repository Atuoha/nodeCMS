'use strict';const express=require("express"),router=express.Router(),Post=require("../../models/Post"),faker=require("faker"),{isEmpty,uploadDir}=require("../../helpers/upload-helper"),path=require("path"),fs=require("fs"),Category=require("../../models/Category"),{userAuth}=require("../../helpers/authenticate");router.all("/*",(a,d,c)=>{a.app.locals.layout="admin";c()});router.get("/",(a,d)=>{Post.find({}).populate("category").then(c=>{d.render("admin/posts",{posts:c})}).catch(c=>console.log("Post Error: err"))});
